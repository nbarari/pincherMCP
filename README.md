...
| **v0.57.0** | **Next dogfood iteration.** Type-info-gated resolver precision — local-variable→declared-type extraction to kill the residual struct-field-read false binds ([#760](https://github.com/kwad77/pincher/issues/760), [#758](https://github.com/kwad77/pincher/issues/758)). Non-Go edge resolution — C/TypeScript corpora currently extract symbols but zero edges, so graph tools are Go-only in practice ([#858](https://github.com/kwad77/pincher/issues/858)). Continued probe-driven hardening. | 🚧 in flight |
...
- **Edge graph is Go-only in practice ([#858](https://github.com/kwad77/pincher/issues/858)).** Cross-file `IMPORTS`/`CALLS`/`READS` resolution runs only for Go. C, TypeScript, and the other regex/JS-AST-tier languages extract symbols fine but produce **zero edges** — so `trace`, `dead_code`, `neighborhood`, and the closure-table fast-path return empty-but-valid results on non-Go repos. `search`, `query` (node scans), `symbol`, and `context` work across every language; only the graph-shaped tools are affected. Multi-language edge resolution is tracked in [#268](https://github.com/kwad77/pincher/issues/268).
