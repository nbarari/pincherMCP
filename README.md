...
| **v0.85** | **Performance push.** Measurement-driven. Per-stage resolve-pass observability ([#1613](https://github.com/kwad77/pincher/issues/1613)) — every resolver (IMPORTS / CALLS / READS / USES_VAR) emits a `pincher.resolve.summary` slog.Info line carrying kind / pending_in / resolved_out / dropped / duration_ms; quiet on no-op ticks. Committed `BenchmarkRebuildFTS_*` coverage for the FTS5 escape hatch ([#1612](https://github.com/kwad77/pincher/issues/1612)) so the bench gate defends a real floor on the previously-unmeasured path. Both PRs are foundation pieces; the parallelism rewrite ([#1611](https://github.com/kwad77/pincher/issues/1611)) and FTS5 fragmentation advisory (rest of #1612) become dogfood-validated follow-ups after the observability numbers come back. | 🚧 in flight |
