...
- **`internal/db/db.go`** — SQLite store. Schema lives here as a `schema` const. Migrations in `schemaMigrations` slice — append to add. Current schema: **v32**. `symSelectFrom` is the canonical SELECT column list — update it and all scan functions together when adding columns. See `docs/REFERENCE.md` for the per-version migration history; this line was 7 versions stale before #998/#999 caught it (and 2 versions stale again before #1418 added a parity test).
