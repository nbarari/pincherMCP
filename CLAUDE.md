...
- **Single shared FTS5 corpus** (mitigation in flight): The legacy `symbols_fts` mixes all symbol kinds, so adding YAML Settings or markdown headings dilutes BM25 scores for code identifiers. **#32 part 1 landed in v9** — three new vtabs (`symbols_code_fts` / `symbols_config_fts` / `symbols_docs_fts`) populate alongside legacy. **Part 2 (next)** adds a `corpus=` parameter on `SearchSymbols`. **Part 3** flips the default and deprecates legacy. Today the new vtabs exist but no caller queries them — zero observable change.
