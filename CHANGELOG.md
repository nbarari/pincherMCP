...
## [0.88.0] — 2026-05-19 — Host conformance + per-host docs (MCP-surface deferrals for v1.0 stability)

**Theme — v0.88 verifies and documents the existing surface; it ships zero new MCP-protocol surface.** The release was originally scoped around the MCP capability trio (resources / completion / icons). A pre-1.0-vs-post-1.0 review settled it on the principle *"1.0 is about stability"*: v0.84 was the explicit API-freeze checkpoint, and adding new MCP capabilities post-freeze would either undermine the freeze or — worse — freeze an under-soaked, possibly half-built resource layer permanently into v1.0. The trio (#1083 / #1084 / #1079) was deferred to v1.1. With it out, v0.88 centers on proving the existing surface across the hosts users actually run, and documenting what each host's user gets.

What shipped: the host-conformance corpus extended from one host to all six (cursor / codex / jetbrains / vscode-copilot / zed join claude-code), per-host benefits pages in `docs/integrations/`, and the Python AST persistent subprocess promoted to default-on with a committed bench gate measuring its ~126× steady-state amortisation.

### Deferred for v1.0 stability

Decisions recorded this release window — each one keeps v1.0's frozen surface soaked rather than expanded:

- **MCP capability trio → v1.1** (#1083 resources, #1084 completion, #1079 icons). New MCP-protocol surface; deferred so v1.0 freezes a soaked tools-only surface.
- **Swift AST → v1.1** (#1452). It can't mirror the Python #856 pattern — SwiftSyntax is a compiled package, not an embed-able script, so it forces a distribution-model change (bundled helper binary / build-on-first-use / tree-sitter+CGo). The 0.85 regex tier is the honest v1.0 Swift answer. The architecture decision is consolidated into a new ADR-decision issue (#1689) covering Swift / Rust / Java AST together.
- **#1371 PK widening → v1.0** — a risk/benefit review found a symbols-table PK rebuild is corruption-class risk for a convenience win; the Phase 2a advisory is an acceptable interim.
- **#640 hook conversion-rate → v1.1** — the "≥20 installs" field measurement is structurally post-launch work; v1.0 records the 60% threshold as a prototype estimate to validate against real field data later.

### DOGFOOD

v0.88 was a planned release without the live-advisory cascade of v0.86–v0.87, so the friction found-and-fixed this window came from auditing the issue tracker rather than from running pincher:

- **#1477 was orphaned in a never-cut `v0.78.2` patch milestone.** v0.78.1 was the last .78 patch; .2 was created, had one issue closed under it, and the other (#1477) sat for weeks. The JS-AST-confidence bug it reported was already fixed — the real gap was a missing *end-to-end* indexer-pipeline regression test (the existing test exercised the AST path in isolation). Added the end-to-end test, rehomed the issue, closed the dead `v0.78.2` milestone.
- **`v1.0` and `v1.0.0` were duplicate milestones.** Issues were splitting across both. Consolidated into `v1.0` (where the roadmap issues live), closed the empty `v1.0.0` duplicate.
- **GitHub's PR-body auto-close only fires per-issue with the keyword repeated.** `Closes #1562, #1563, …` closed only #1562; the host-conformance sibling issues #1563–#1566 had to be closed manually. Noted for future multi-issue PRs.
